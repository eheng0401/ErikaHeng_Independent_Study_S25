...
dataFolder = 'acousticbrainz-highlevel-json-20220623'

def process_json_files(dataFolder, csv_fname):
    main_folder = Path(dataFolder)
    with open(csv_fname, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(["Title", "MusicBrainz Recording ID", "Danceability",  # data headers
        ...
        for json_file in main_folder.rglob("*.json"):  # Searches all JSON files in subdirectories
            try:
                with open(json_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)  
                tags = data["metadata"]["tags"]  # metadata and tags always exist no error h
                highLevel = data["highlevel"]
                if "title" in tags and "musicbrainz_recordingid" in tags: 
                    row = []
                    title = tags["title"]
...
def main():
    csv_fname = 'sample_data_1.csv'
    process_json_files(dataFolder, csv_fname)
