...
def preprocess_data(df, features):
    ...
    lyrics_data = df['lyrics'].fillna('') #add empty string
    ...
    lyrics_features_scaled = MinMaxScaler().fit_transform(lyrics_features)  # scaling lyrics
    combined_features = np.hstack([normalized_features, lyrics_features_scaled])
    extended_numeric_features = numeric_features + [f'lyrics_feature_{i}' for i in range(lyrics_features_scaled.shape[1])] 
...
def apply_boolean_filters(weights, danceability, genre, mood, sound, lyrics):
    ...
    bool_mappings = {
        ...
        lyrics: list(range(58, length))  # lyrics features are at the end
    }
...
def main(search_title, danceability=False, genre=False, mood=False, sound=False, lyrics=False):
    ...
    features = ["Title", "MusicBrainz Recording ID", "Danceability", # dancability
                "genre_dortmund", "dort_alternative", "dort_blues", "dort_electronic", "dort_folkcountry","dort_funksoulrnb", "dort_jazz", "dort_pop", "dort_raphiphop", "dort_rock", # genre stuff 
                ...
                "mood_acoustic", "mood_aggressive", "mood_electronic", "mood_happy", "mood_party", "mood_relaxed", "mood_sad", # mood filters
